...
class ImportFileService:
    ...
    def save_upload_file_to_local(self, file: UploadFile, file_dir: str) -> str:
        ...
        # 2. 构建上传文件完整的path
        import_file_path = os.path.join(file_dir, str(file.filename))
    ...
    def save_upload_file_to_minio(self, import_file_path: str, file: UploadFile) -> None:
        '''
        将上传的文件保存到MinIO
        '''
        # 1. 获取minio客户端
    ...
    def process_upload_file(self, file: UploadFile):
        ...
        2. 将上传的文件保存到本地
        3. 将上传的文件保存到MinIO
        4. 标记当前文件上传节点（'upload_file'）为运行完成
        5. 返回（task_id, file_dir, import_file_path）
        '''
    ...
    def run_import_graph(self, task_id: str, file_dir: str, import_file_path: str):
        ...
        2. 运行(graph.stream())
            ...
            2.2 在运行图的某个节点之后调用mark_node_done
        3. update_task_status更新任务的状态为completed
        '''
        try:
            # 1. 标记任务开始处理
