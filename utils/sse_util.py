...
def push_sse_event(task_id: str, event: str, data):
    '''推送 SSE 事件到队列'''
    # 1. 获取 SSE 队列
...
async def sse_generator(task_id: str, request: Request) -> AsyncGenerator[str, None]:
    '''
    SSE 生成器，用于 FastAPI 的 StreaminigResponse
    '''
    # 1. 获取队列
    stream_queue = get_sse_queue(task_id)
    ...
    try:
        ...
        while True:
            ...
            # 9. 讲正常的事件和数据打包成标准的 SSE 格式，并通过 yield 推送给前端客户端
            yield _sse_pack(event, data)
    
    except (ConnectionResetError, BrokenPipeError):
        # 客户端强行刷新页面或关闭标签页，TCP管道破裂，静默退出
        return
        
    except asyncio.CancelledError:
        # 协程被取消：重新抛出，让外层框架知道被成功取消
        raise
